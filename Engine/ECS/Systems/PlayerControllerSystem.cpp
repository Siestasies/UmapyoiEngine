...
namespace Uma_ECS
{
	...
	void PlayerControllerSystem::HandleMovementInput(float dt)
	{
        ...
        if (inputState.moveRight) targetAccel.y += rb.accel_strength;
        if (inputState.moveLeft	) targetAccel.y -= rb.accel_strength;
        if (inputState.moveUp	) targetAccel.x += rb.accel_strength;
        if (inputState.moveDown	) targetAccel.x -= rb.accel_strength;
