...
namespace Uma_ECS
{
    ...
    struct LuaScript
    {
        std::vector<std::unique_ptr<LuaScriptInstance>> scripts;
        ...
        LuaScriptInstance* GetScript(size_t idx)
        {
            if (idx < scripts.size())
            {
                return scripts[idx].get();  // ← Return raw pointer
            }
        ...
        LuaScriptInstance* GetScriptByPath(const std::string& path)
        {
            for (auto& script : scripts)
            {
                ...
                {
                    return script.get();  // Return raw pointer
                }
            }
        ...
        void Serialize(rapidjson::Value& value, rapidjson::Document::AllocatorType& allocator) const
        {
            ...
            for (const auto& script : scripts)
            {
                ...
                script->Serialize(scriptObj, allocator);  // Dereference unique_ptr
