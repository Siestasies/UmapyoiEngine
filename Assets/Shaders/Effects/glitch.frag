...
void main()
{
    // Pseudo-random based on time and vertical position
    ...
    // RGB channel split
    float r = texture(image, clamp(uv + vec2(intensity * 0.3 * noise, 0.0), 0.0, 1.0)).r;
    float g = texture(image, uv).g;
    float b = texture(image, clamp(uv - vec2(intensity * 0.3 * noise, 0.0), 0.0, 1.0)).b;
